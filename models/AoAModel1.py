...
from .TransformerModel1 import LayerNorm, attention, attention_d, clones, SublayerConnection, PositionwiseFeedForward
...
class MultiHeadedDotAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, scale=1, project_k_v=1, use_output_layer=1, do_aoa=0, norm_q=0,
        ...
        self.k_linears = clones(nn.Linear(d_model, d_model * scale), 3)  # (parent,neighbor,child) so copy three times
...
class AoA_Decoder_Core(nn.Module):
    def __init__(self, opt):
        ...
        if opt.use_multi_head == 2:
            self.attention = MultiHeadedDotAttention_d(opt.num_heads, opt.rnn_size, project_k_v=0,
                                                       scale=opt.multi_head_scale, use_output_layer=0, do_aoa=0,
                                                       norm_q=1)
        else:
...
class AoAModel1(AttModel):
    def __init__(self, opt):
        super(AoAModel1, self).__init__(opt)
        self.num_layers = 2
        # mean pooling
