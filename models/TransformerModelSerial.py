...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, src_attn, src_rel_attn, feed_forward, dropout):
    ...
    def forward(self, x, obj_memory, rel_memory, src_mask, src_rel_mask, tgt_mask):
        ...
        x = self.sublayer[2](x, lambda x: self.src_rel_attn(x, rm, rm, src_rel_mask))
...
class TransformerModelSerial(AttModel_rel):

    def make_model(self, src_vocab, tgt_vocab, N=6, 
               d_model=512, d_ff=2048, h=8, dropout=0.1):  # N: number of encoder/decoder layer, d_model: dim of entire model, d_ff: dim of model's input, h: number of identical heads of self-attention
        ...
        model = EncoderDecoder(
            ...
            Decoder(DecoderLayer(d_model, c(attn), c(attn), c(attn), c(ff), dropout), N),
            lambda x:x, # nn.Sequential(Embeddings(d_model, src_vocab), c(position)),
            lambda x: x,  # nn.Sequential(Embeddings(d_model, src_rel_vocab), c(position)),
    ...
    def __init__(self, opt):
        super(TransformerModelSerial, self).__init__(opt)
        self.opt = opt
