...
from .TransformerModel3_d1_24heads import LayerNorm, attention, attention_d, clones, SublayerConnection, PositionwiseFeedForward
# for decoder
...
class MultiHeadedDotAttention_d(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, scale=1, project_k_v=1, use_output_layer=1, do_aoa=0, norm_q=0, dropout_aoa=0.3):
        ...
        self.linears = clones(nn.Linear(d_model, d_model * scale), 3)
    ...
    def forward(self, query, x1, x2, x3, mask=None):
        ...
        if self.project_k_v == 0:
            query1_ = self.linears[0](query).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            query2_ = self.linears[1](query).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            query3_ = self.linears[2](query).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
        ...
        # print(value_.shape)
        # print(mask.shape)
        x, self.attn = attention_d(query1_, query2_, query3_, key1_, key2_, key3_, value1_, value2_, value3_, mask=mask,
