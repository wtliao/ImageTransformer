...
class MultiHeadedDotAttention(nn.Module):
    ...
    def forward(self, query, value, key, flag, mask=None):
        ...
        if self.project_k_v == 0:
            ...
            key_ = self.k_linears(value).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            value_ = self.v_linears(key).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
...
class AoA_Refiner_Core(nn.Module):
    def __init__(self, opt):
        ...
        self.layers = clones(layer, getattr(opt, 'aoa_num', 6))  # NOTE: number of aoa module is an inportant
...
class AoAModel_b_old(AttModel):
    def __init__(self, opt):
        super(AoAModel_b_old, self).__init__(opt)
        self.num_layers = 2
        # mean pooling
