...
from .TransformerModel3_d1_w4 import LayerNorm, attention, attention_d, clones, SublayerConnection, PositionwiseFeedForward
# for decoder
...
class MultiHeadedDotAttention_d(nn.Module):
    ...
    def forward(self, query, x1, x2, x3, x4, mask=None):
        if mask is not None:
            if len(mask.size()) == 2:
        ...
        key4 = x4.narrow(2, 0, 1024)
        value4 = x4.narrow(2, 1024, 1024)
        ...
        if self.project_k_v == 0:
            ...
            key4_ = key4.view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            value4_ = value4.view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
        else:
            query_, key_, value_ = \
        ...
        # print(value_.shape)
        # print(mask.shape)
        x, self.attn = attention_d(query_, key1_, key2_, key3_, key4_, value1_, value2_, value3_, value4_, mask=mask,
...
class AoA_Decoder_Core(nn.Module):
    def __init__(self, opt):
        ...
        self.implicit4 = nn.Linear(1024, 2048)
    ...
    def forward(self, xt, mean_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        p_att_feats4 = self.implicit4(att_feats)
        ...
        if self.use_multi_head == 2:
            ...
            att = self.attention(h_att, p_att_feats1, p_att_feats2, p_att_feats3, p_att_feats4, att_masks)
        else:
...
class AoAModel3_d1_w4(AttModel):
    def __init__(self, opt):
        super(AoAModel3_d1_w4, self).__init__(opt)
        self.num_layers = 2
        # mean pooling
