...
class AttModel_rel(CaptionModel):
    def __init__(self, opt):
        super(AttModel_rel, self).__init__()
        ...
        self.rel_feat_size = opt.rel_feat_size
    ...
    def _prepare_rel_feature(self, rel_feats, rel_masks):
        ...
        # Project the attention feats first to reduce memory and computation comsumptions.
        p_rel_feats = self.ctx2att(rel_feats)

        return rel_feats, p_rel_feats, rel_masks
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, rel_feats, p_att_feats, p_rel_feats, att_masks, rel_masks, state):
        # 'it' contains a word index
        xt = self.embed(it)

        output, state = self.core(xt, fc_feats, att_feats, rel_feats, p_att_feats, p_rel_feats, state, att_masks, rel_masks)
        logprobs = F.log_softmax(self.logit(output), dim=1)

        return logprobs, state

    def _sample_beam(self, fc_feats, att_feats, rel_feats, att_masks=None, rel_masks=None, opt={}):
        beam_size = opt.get('beam_size', 10)
        batch_size = fc_feats.size(0)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, att_feats, att_masks)
        p_rel_feats, pp_rel_feats, p_rel_masks = self._prepare_rel_feature(rel_feats, rel_masks)
        ...
        for k in range(batch_size):
            ...
            tmp_rel_feats = p_rel_feats[k:k + 1].expand(*((beam_size,) + p_rel_feats.size()[1:])).contiguous()
            tmp_p_att_feats = pp_att_feats[k:k + 1].expand(*((beam_size,) + pp_att_feats.size()[1:])).contiguous()
            tmp_p_rel_feats = pp_rel_feats[k:k + 1].expand(*((beam_size,) + pp_rel_feats.size()[1:])).contiguous()
            tmp_att_masks = p_att_masks[k:k + 1].expand(*((beam_size,) + p_att_masks.size()[1:])).contiguous() if att_masks is not None else None
            tmp_rel_masks = p_rel_masks[k:k + 1].expand(*((beam_size,) + p_rel_masks.size()[1:])).contiguous() if rel_masks is not None else None

            for t in range(1):
                if t == 0:  # input <bos>
                    it = fc_feats.new_zeros([beam_size], dtype=torch.long)

                logprobs, state = self.get_logprobs_state(it, tmp_fc_feats, tmp_att_feats, tmp_rel_feats,
                                                          tmp_p_att_feats, tmp_p_rel_feats, tmp_att_masks, tmp_rel_masks, state)

            self.done_beams[k] = self.beam_search(state, logprobs, tmp_fc_feats, tmp_att_feats, tmp_rel_feats,
                                                  tmp_p_att_feats, tmp_p_rel_feats, tmp_att_masks, tmp_rel_masks, opt=opt)
            seq[:, k] = self.done_beams[k][0]['seq']  # the first beam has highest cumulative score
            seqLogprobs[:, k] = self.done_beams[k][0]['logps']
        # return the samples and their log likelihoods
        return seq.transpose(0, 1), seqLogprobs.transpose(0, 1)

    def _sample(self, fc_feats, att_feats, rel_feats, att_masks=None, rel_masks=None, opt={}):
        ...
        if beam_size > 1:
            return self._sample_beam(fc_feats, att_feats, rel_feats, att_masks, rel_masks, opt)
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, p_rel_feats, pp_att_feats, pp_rel_feats, p_att_masks, p_rel_masks, state)
