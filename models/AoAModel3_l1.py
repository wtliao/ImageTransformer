...
from .TransformerModel3_l1 import LayerNorm, attention, attention_d, clones, SublayerConnection, PositionwiseFeedForward
# for decoder
...
class AoA_Refiner_Layer1(nn.Module):
    def __init__(self, size, self_attn, feed_forward, dropout):
        super(AoA_Refiner_Layer1, self).__init__()
        self.self_attn = self_attn
        self.feed_forward = feed_forward
        self.use_ff = 0
        if self.feed_forward is not None:
...
class AoA_Refiner_Core(nn.Module):
    def __init__(self, opt):
        ...
        attn1 = MultiHeadedDotAttention_d(opt.num_heads, opt.rnn_size, project_k_v=1, scale=opt.multi_head_scale, do_aoa=opt.refine_aoa, norm_q=0, dropout_aoa=getattr(opt, 'dropout_aoa', 0.3))
        self.layer1 = AoA_Refiner_Layer(opt.rnn_size, attn, PositionwiseFeedForward(opt.rnn_size, 2048, 0.1) if opt.use_ff else None, 0.1)
        self.layer2 = AoA_Refiner_Layer1(opt.rnn_size, attn1, PositionwiseFeedForward(opt.rnn_size, 2048, 0.1) if opt.use_ff else None, 0.1)
        self.layer3 = AoA_Refiner_Layer1(opt.rnn_size, attn1, PositionwiseFeedForward(opt.rnn_size, 2048, 0.1) if opt.use_ff else None, 0.1)
        #self.layers = clones(layer, 3)
        self.norm = LayerNorm(self.layer3.size)

    def forward(self, x, flag, mask):
        x = self.layer1(x, flag, mask)
...
class AoAModel3_l1(AttModel):
    def __init__(self, opt):
        super(AoAModel3_l1, self).__init__(opt)
        self.num_layers = 2
        # mean pooling
