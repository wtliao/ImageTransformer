...
class MultiHeadedDotAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, scale=1, project_k_v=1, use_output_layer=1, do_aoa=0, norm_q=0, dropout_aoa=0.3):
        ...
        self.h = h
        # Note to know the meaning of each parameters
        ...
        self.linears = clones(nn.Linear(d_model, d_model * scale), 1 + 2 * project_k_v) #Note ?????for what
        ...
        self.use_aoa = do_aoa
        if self.use_aoa:
            self.aoa_layer = nn.Sequential(nn.Linear((1 + scale) * d_model, 2 * d_model), nn.GLU()) # Note to understand aoa layer
    ...
    def forward(self, query, value, key, mask=None):
        ...
        if self.use_aoa:
            # Apply AoA, this is the core and only one difference from transformer!!!
...
class AoA_Refiner_Layer(nn.Module):
    # NOTE: the encoder refine block in figure 4
...
class AoA_Refiner_Core(nn.Module):
    def __init__(self, opt):
        ...
        layer = AoA_Refiner_Layer(opt.rnn_size, attn, PositionwiseFeedForward(opt.rnn_size, 2048, 0.1) if opt.use_ff   
                                  else None, 0.1)
...
class AoA_Decoder_Core(nn.Module):
    def __init__(self, opt):
        ...
        if opt.use_multi_head == 2:
            self.attention = MultiHeadedDotAttention(opt.num_heads, opt.rnn_size, project_k_v=0, scale=opt.multi_head_scale, use_output_layer=0,
                                                     do_aoa=0, norm_q=1)
        else:
    ...
    def forward(self, xt, mean_feats, att_feats, p_att_feats, state, att_masks=None):
        """
        att_feats: obj features from encoder, not used in aoa decoder
        mean_feats: mean of all obj features, used as global context
        p_att_feats: att_feats after self.ctx2att
        """
        # state[0][1] is the context vector at the last step, and added to mean_feats(global context), as described in Fig. 5
        ...
        if self.use_multi_head == 2:
            att = self.attention(h_att, p_att_feats.narrow(2, 0, self.multi_head_scale * self.d_model),
                                 p_att_feats.narrow(2, self.multi_head_scale * self.d_model, self.multi_head_scale * self.d_model), att_masks)
        else:
            att = self.attention(h_att, att_feats, p_att_feats, att_masks)

        ctx_input = torch.cat([att, h_att], 1) # for AoA operation
        if self.decoder_type == 'LSTM':
            output, c_logic = self.att2ctx(ctx_input, (state[0][1], state[1][1]))
            state = (torch.stack((h_att, output)), torch.stack((c_att, c_logic)))
        else:
            output = self.att2ctx(ctx_input) # AoA operation
...
class AoAModel(AttModel):
    def __init__(self, opt):
        ...
        if opt.use_multi_head == 2:
            del self.ctx2att
            self.ctx2att = nn.Linear(opt.rnn_size, 2 * opt.multi_head_scale * opt.rnn_size) # what is ctx2attn

        if self.use_mean_feats:
            del self.fc_embed
        if opt.refine:
            self.refiner = AoA_Refiner_Core(opt) # encoder
        else:
            self.refiner = lambda x, y: x
        self.core = AoA_Decoder_Core(opt)  # decoder core
