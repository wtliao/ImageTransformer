...
from .TransformerModel3_d1 import LayerNorm, attention, attention_d, clones, SublayerConnection, PositionwiseFeedForward
# for decoder
...
class MultiHeadedDotAttention_d(nn.Module):
    ...
    def forward(self, query, x1, x2, x3, mask=None):
        if mask is not None:
            if len(mask.size()) == 2:
        ...
        x, self.attn = attention_d(query_, key1_, key2_, key3_, value1_, value2_, value3_, mask=mask,
                                   dropout=self.dropout)
        # print(x.shape)

        # TODO: save the output of attention_d for analyze
        # import ipdb; ipdb.set_trace()
        # if not self.old_img_id == self.img_info['id']:
        #     self.decode_step = 0  # new image reset the step counter
        #     self.old_img_id = self.img_info['id']
        # p_attn = {'p_attn1': self.attn.data.cpu().numpy(), 'p_attn2': p_attn2.data.cpu().numpy(), 'p_attn3': p_attn3.data.cpu().numpy()}
        # with open('/home/liao/work_code/AoANet/data/tmp/attention_d/' + str(self.img_info['id']) + '_s{}'.format(self.decode_step) + '.pkl', 'wb') as f:
        #     utils.pickle_dump(p_attn, f)
        # self.decode_step += 1
        # "Concat" using a view
...
class MultiHeadedDotAttention(nn.Module):
    ...
    def forward(self, query, value, key, flag, mask=None):
        ...
        # print(mask.shape)
        # import ipdb; ipdb.set_trace()
        x, self.attn, p_attn2, p_attn3 = attention(query_, key_p, key_n, key_c, value_p, value_n, value_c, flag,                                                    mask=mask, dropout=self.dropout)
        # print(x.shape)
        # TODO: save the output of attention_d for analyze
        ...
        #         with open('/home/liao/work_code/AoANet/data/tmp/attention_value_test/'+str(img_info['id'])+'.pkl', 'wb') as f:
        #             utils.pickle_dump(v, f)
...
class AoA_Refiner_Core(nn.Module):
    def __init__(self, opt):
        ...
        self.layers = clones(layer, getattr(opt, 'aoa_num', 6))  # NOTE: number of aoa module is an important hyper-parameter, if not specified, use the most popular 6
...
class AoAModel3_d1(AttModel):
    def __init__(self, opt):
        super(AoAModel3_d1, self).__init__(opt)
        self.num_layers = 2
        self.batch_info = []
        # mean pooling
    ...
    def _prepare_feature(self, fc_feats, att_feats, flag_feats, att_masks):
        ...
        att_feats, att_masks, flag_feats = self.clip_att(att_feats, att_masks, flag_feats)
        # print(att_feats.shape)
        # embed att feats
