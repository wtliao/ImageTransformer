...
from .TransformerModel3_d3 import LayerNorm, attention, attention_d, clones, SublayerConnection, PositionwiseFeedForward
# for decoder
...
class MultiHeadedDotAttention_d(nn.Module):
    ...
    def forward(self, query, x1, x2, x3, mask=None):
        ...
        # print(value_.shape)
        # print(mask.shape)
        x1, x2, x3, self.attn = attention_d(query_, key1_, key2_, key3_, value1_, value2_, value3_, mask=mask,
        ...
        x3 = x3.transpose(1, 2).contiguous() \
            .view(nbatches, -1, self.h * self.d_k)
        # print(x1.shape)
        gate_fea = torch.cat([query, x1, x2, x3], -1)
        gate = self.sig(self.gate(gate_fea))
        fea = gate[:, :, 0] * x1.squeeze(1) + gate[:, :, 1] * x2.squeeze(1) + gate[:, :, 2] * x3.squeeze(1)
        # print(gate_fea.shape)
        # print(gate.shape)
        # print(gate[:,:,0].shape)
        # print(fea.shape)
        ...
        if self.use_aoa:
            # Apply AoA
            x = self.aoa_layer(self.dropout_aoa(torch.cat([x1, query], -1)))

        x = self.output_layer(fea)
...
class AoAModel3_d3(AttModel):
    def __init__(self, opt):
        super(AoAModel3_d3, self).__init__(opt)
        self.num_layers = 2
        # mean pooling
