# Use val/test set to refine the model for final challenge test
GPU_id=5
...
# start_from=""
start_from="--start_from log/tmp/train_ours/log_refine_aoa_${id}_aoa${aoa_num}_new7_all"
...
# 	--input_label_h5 data/tmp/5/cocotalk_label.h5 \
...
# 	--name_append ""\
# 	--checkpoint_path log/tmp/train_ours/log_refine_aoa_${id}_aoa${aoa_num}_new7_all\
# 	$start_from \
...
python train_h2.py --id $id --refine_aoa $refine_aoa --caption_model $caption_model --aoa_num $aoa_num\
	...
	--save_checkpoint_every 10000 \
	--name_append "65"\
	--start_from log/tmp/train_ours/log_refine_aoa_${id}_aoa${aoa_num}_new6_all_rl\
	--checkpoint_path log/tmp/train_ours/log_refine_aoa_${id}_aoa${aoa_num}_new6_all_rl \
	--learning_rate 2e-5 \
	--max_epochs 75 \
