...
class DataLoader(data.Dataset):
    ...
    def get_batch(self, split, batch_size=None):
        ...
        data['flag_feats'] = np.zeros([len(flag_batch) * seq_per_img, max_att_len, 4])
        for i in range(len(att_batch)):
            data['att_feats'][i * seq_per_img:(i + 1) * seq_per_img, :att_batch[i].shape[0]] = att_batch[i]
        for i in range(len(att_batch)):
            data['flag_feats'][i * seq_per_img:(i + 1) * seq_per_img, :flag_batch[i].shape[0], :4] = flag_batch[i]
