...
def train(opt):
    ...
    if opt.start_from is not None:
        ...
        histories_path = os.path.join(opt.start_from, 'histories_' + opt.id + opt.name_append + '.pkl')
    ...
    def save_checkpoint(model, infos, optimizer, histories=None, append=''):
        ...
        with open(os.path.join(opt.checkpoint_path, 'infos_' + opt.id + '%s.pkl' % (append)), 'wb') as f:
            ...
            print("Save training information to {}".format(os.path.join(opt.checkpoint_path, 'infos_' + opt.id + '%s.pkl' % (append))))
            ...
            with tqdm(total=len(loader.split_ix['train']), initial=start_Img_idx) as pbar:
                for i in range(start_Img_idx, len(loader.split_ix['train']), opt.batch_size):
                    ...
                    if data['bounds']['wrapped']:
                        ...
                        save_checkpoint(model, infos, optimizer, append=str(epoch - 1))
...
opt.start_from='log/tmp/log_aoanet'
print("========================start from {}.".format(opt.start_from))
train(opt)
